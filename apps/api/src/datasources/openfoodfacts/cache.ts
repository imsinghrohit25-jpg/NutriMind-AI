...
function pg(v: number | null): number | null {
  if (v === null) return null;
  const n = Number(v);
  return isFinite(n) ? n : null;
}

function rowToNutrition(row: NutritionRow): NutritionPer100g {
  ...
  return {
    ...
    energyKcal: pg(Number(row.energy_kcal)),
    energyKj: pg(Number(row.energy_kj)),
    proteinG: pg(Number(row.protein_g)),
    fatTotalG: pg(Number(row.fat_total_g)),
    fatSaturatedG: pg(Number(row.fat_saturated_g)),
    fatTransG: pg(Number(row.fat_trans_g)),
    fatPolyunsaturatedG: pg(Number(row.fat_polyunsaturated_g)),
    fatMonounsaturatedG: pg(Number(row.fat_monounsaturated_g)),
    carbohydratesG: pg(Number(row.carbohydrates_g)),
    sugarsG: pg(Number(row.sugars_g)),
    sugarsAddedG: pg(Number(row.sugars_added_g)),
    sugarsAddedEstimated: row.sugars_added_estimated,
    dietaryFiberG: pg(Number(row.dietary_fiber_g)),
    sodiumMg: pg(Number(row.sodium_mg)),
    cholesterolMg: pg(Number(row.cholesterol_mg)),
    calciumMg: pg(Number(row.calcium_mg)),
    ironMg: pg(Number(row.iron_mg)),
    potassiumMg: pg(Number(row.potassium_mg)),
    zincMg: pg(Number(row.zinc_mg)),
    vitaminCMg: pg(Number(row.vitamin_c_mg)),
    vitaminAIu: pg(Number(row.vitamin_a_iu)),
    vitaminDIu: pg(Number(row.vitamin_d_iu)),
    vitaminB12Mcg: pg(Number(row.vitamin_b12_mcg)),
    folateMcg: pg(Number(row.folate_mcg)),
    novaGroup: novaRaw === 1 || novaRaw === 2 || novaRaw === 3 || novaRaw === 4 ? novaRaw : null,
    confidence: pg(Number(row.confidence)),
    notes: row.notes,
  };
}
